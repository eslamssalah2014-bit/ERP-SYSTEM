...
import {
  ...
  FileText, CheckCircle2, ShoppingBag, ArrowRight, Printer, Receipt
} from "lucide-react";
...
export default function PurchaseReturnsPage() {
  ...
  const [refundMethod, setRefundMethod] = useState<"supplier_balance" | "treasury">("supplier_balance");
  const [treasuryAccountId, setTreasuryAccountId] = useState("");
  const [notes, setNotes] = useState("");
  const [items, setItems] = useState<Omit<PurchaseReturnItem, "id">[]>([]);
  ...
  const handleOpenAddModal = () => {
    ...
    setRefundMethod("supplier_balance");
    setNotes("");
    setItems([]);
  ...
  const handleCreateReturn = async (e: React.FormEvent) => {
    ...
    if (!supp) {
      setFormError(isAr ? "يرجى اختيار المورد" : "Please select a supplier");
      return;
    }

    if (items.length === 0) {
      setFormError(isAr ? "يرجى إضافة صنف واحد على الأقل للمرتجع" : "Please add at least one line item");
      return;
    }
    ...
    try {
      const returnNumber = `PRET-${new Date().getFullYear()}-${(purchaseReturns.length + 1).toString().padStart(4, "0")}`;
      const originalInv = purchaseInvoices.find(i => i.id === originalInvoiceId);

      const created = await addPurchaseReturn({
        ...
        notes: notes || (isAr ? "مرتجع مشتريات إلى المورد وخصم من المخزن" : "Purchase Return"),
      ...
      showToast(
        isAr ? `تم تسجيل مرتجع المشتريات (${returnNumber}) وخصم المخزون بنجاح` : `Purchase return (${returnNumber}) registered`,
        "success"
      );
    } catch (err: any) {
      console.error("Failed to create purchase return:", err);
      const errMsg = err?.message || (isAr ? "فشل تسجيل مرتجع المشتريات" : "Failed to record return");
      setFormError(errMsg);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(isAr ? "هل أنت متأكد من حذف مرتجع المشتريات؟" : "Are you sure you want to delete this return?")) return;
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-sm">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <a href="/purchases" className="text-slate-400 hover:text-white transition-colors flex items-center gap-1 text-xs">
              <ShoppingBag className="w-3.5 h-3.5" />
              <span>{isAr ? "فواتير المشتريات" : "Purchase Invoices"}</span>
            </a>
            <span className="text-slate-600">/</span>
            <span className="text-rose-400 font-bold text-xs">{isAr ? "مرتجعات المشتريات" : "Returns"}</span>
          </div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2.5">
            <RotateCcw className="w-6 h-6 text-rose-400" />
            <span>{isAr ? "مرتجعات المشتريات (الإشعارات المدينة - Debit Notes)" : "Purchase Returns & Debit Notes"}</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            {isAr ? "إدارة رد البضائع للموردين وخصم الكميات من المخزون وتسوية مستحقات المورد / الخزينة" : "Process vendor returns with inventory deductions and AP adjustments"}
          </p>
        </div>

        <button
          onClick={handleOpenAddModal}
          className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-rose-600 to-amber-500 hover:opacity-95 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-950/60 transition-all cursor-pointer"
        >
          <Plus className="w-4 h-4" />
          <span>{isAr ? "إضافة مرتجع مشتريات جديد" : "New Purchase Return"}</span>
        </button>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder={isAr ? "بحث برقم المرتجع أو الفاتورة أو المورد..." : "Search return # or supplier..."}
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-slate-950 border border-slate-800 rounded-xl pr-10 pl-4 py-2 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-rose-500"
          />
        </div>
      </div>
          ...
          <table className="w-full text-xs text-right border-collapse">
            <thead>
              <tr className="bg-slate-800/80 text-slate-400 font-bold border-b border-slate-700">
                ...
                <th className="p-3.5">{isAr ? "فاتورة الشراء الأصلية" : "Original Purchase Inv"}</th>
                ...
                <th className="p-3.5 text-center font-mono">{isAr ? "إجمالي الإشعار المدين" : "Total Refund"}</th>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {filteredReturns.map((ret, idx) => (
                <tr key={ret.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="p-3.5 font-bold text-slate-200">{ret.supplierName}</td>
                  ...
                  <td className="p-3.5 text-center">
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-800 text-slate-300 border border-slate-700">
                      {ret.refundMethod === "treasury" ? (isAr ? "استلام نقدي في الخزينة" : "Cash Inflow") : (isAr ? "خصم من رصيد المورد" : "Supplier Debit")}
                    </span>
                  </td>
                  <td className="p-3.5 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      <button
                        ...
                        className="p-1.5 bg-slate-800 hover:bg-rose-600 hover:text-white text-slate-300 rounded-lg transition-colors cursor-pointer"
                ...
                <tr>
                  <td colSpan={10} className="text-center py-12 text-slate-500">
                    ...
                    <p className="text-sm font-semibold text-slate-400">
                      {isAr ? "لا توجد مرتجعات مشتريات مسجلة" : "No purchase returns recorded"}
                    </p>
                    <p className="text-xs text-slate-600 mt-1">
                      {isAr ? "اضغط على زر (إضافة مرتجع مشتريات جديد) لتسجيل أول حركة إرجاع للمورد" : "Click 'New Purchase Return' to record a return"}
                    </p>
                  </td>
                </tr>
              )}
      ...
      <Modal
        ...
        title={isAr ? "تسجيل مرتجع مشتريات جديد (إشعار مدين)" : "New Purchase Return (Debit Note)"}
        maxWidth="4xl"
      >
        <form onSubmit={handleCreateReturn} className="space-y-4 text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-950 p-4 rounded-2xl border border-slate-800">
            <div>
              <label className="block text-slate-400 font-semibold mb-1">
                {isAr ? "ربط بفاتورة مشتريات سابقة (اختياري)" : "Original Purchase Invoice"}
              </label>
              <select
                value={originalInvoiceId}
                onChange={(e) => handleSelectOriginalInvoice(e.target.value)}
                className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-rose-500 font-mono"
              >
                <option value="">{isAr ? "-- بدون ربط بفاتورة محددة --" : "-- No Specific Invoice --"}</option>
                {purchaseInvoices.map(i => (
                  <option key={i.id} value={i.id}>
                    {i.invoiceNumber} - {i.supplierName} ({formatCurrency(i.grandTotal, organization.currency, locale)})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label className="block text-slate-400 font-semibold mb-1">{isAr ? "مستودع الصرف (الخصم) *" : "Source Warehouse *"}</label>
              <select
                value={warehouseId}
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-rose-500 font-bold"
              >
                <option value="supplier_balance">{isAr ? "خصم من رصيد المورد / إشعار مدين" : "Debit Supplier Account"}</option>
                <option value="treasury">{isAr ? "استلام نقدي مباشر في الخزينة" : "Cash Inflow to Treasury"}</option>
              </select>
            </div>

            {refundMethod === "treasury" && (
              <div>
                <label className="block text-slate-400 font-semibold mb-1">{isAr ? "الخزينة المستلم فيها *" : "Treasury Account *"}</label>
                <select
          ...
          <div className="border border-slate-800 rounded-2xl overflow-hidden">
            ...
            <div className="p-2 bg-slate-950 flex justify-start">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-rose-400 font-bold rounded-lg transition-colors cursor-pointer"
              >
          ...
          <div className="flex justify-between items-center bg-slate-950 p-4 rounded-2xl border border-slate-800">
            <div className="w-1/2">
              <label className="block text-slate-400 text-[11px] mb-1">{isAr ? "سبب الإرجاع للمورد" : "Return Reason"}</label>
              <input
                type="text"
                placeholder={isAr ? "مثال: عدم مطابقة المواصفات / بضاعة معيبة..." : "Reason for return..."}
                value={notes}
            ...
            <div className="w-72 space-y-1.5 text-right">
              ...
              <div className="flex justify-between text-slate-400">
                <span>{isAr ? `الضريبة المستردة (${organization.defaultVatRate}%):` : "VAT Refunded:"}</span>
              ...
              <div className="flex justify-between text-base font-black text-white pt-2 border-t border-slate-800">
                <span>{isAr ? "إجمالي الإشعار المدين:" : "Total Debit Note:"}</span>
                <span className="font-mono text-rose-400">{formatCurrency(grandTotal, organization.currency, locale)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-800">
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="flex items-center gap-2 px-6 py-2.5 bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold rounded-xl shadow-lg transition-colors cursor-pointer disabled:opacity-50"
            >
              {isSubmitting ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <span>{isAr ? "جاري ترحيل المرتجع وخصم المخزن..." : "Processing Return..."}</span>
                </>
              ) : (
                <span>{isAr ? "تسجيل المرتجع وخصم المخزون" : "Record Return & Deduct Stock"}</span>
              )}
            </button>
          </div>
        </form>
      ...
      {selectedReturn && (
        <Modal
          ...
          title={isAr ? `إشعار مدين مشتريات (${selectedReturn.returnNumber})` : `Debit Note (${selectedReturn.returnNumber})`}
          maxWidth="2xl"
        >
          <div className="p-4 space-y-4 text-xs">
            <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm font-bold text-white">{selectedReturn.supplierName}</p>
              </div>
              <div>
              ...
              <div>
                <p className="text-slate-400">{isAr ? "إجمالي الإشعار" : "Total"}</p>
                ...
                <tbody className="divide-y divide-slate-800">
                  {(selectedReturn.items || []).map((it, i) => (
                    <tr key={i}>
                      ...
                      <td className="p-2.5 text-left font-mono font-bold text-rose-400">{formatCurrency(it.total, organization.currency, locale)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
